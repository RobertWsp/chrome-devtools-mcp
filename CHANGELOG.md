...
* session isolation: browser sessions are now scoped to the host session (pi session) that created them. The host passes a transport-level owner id (reserved `__mcpClientId` arg, never shown to the model); `list_sessions` returns only the caller's own sessions, and operating or closing a session owned by someone else fails with the same generic "not found" as a truly non-existent id — so one session can neither see, use, nor enumerate another session's browsers/tabs. Callers without an owner only reach ownerless sessions. The `flow` tool is owner-scoped through the same single guard, so a caller cannot draft/save/list/replay against another owner's session (which would otherwise leak its recording buffer or project flows). The owner is persisted with `--persist-sessions` and restored on reconnect, so isolation survives a subprocess restart.
